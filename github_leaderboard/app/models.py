...
class Result(models.Model):
    def default_score():
        return 0
    leaderboard = models.ForeignKey('Leaderboard', on_delete=models.CASCADE, null=True, blank=True, related_name='results')    
...
class Leaderboard(models.Model):
    ...
    def get_ranked_user_commit_data(self):
        ...
        ranked_data = Commit.objects.filter(leaderboard=self, user__in=self.participants.all()).values('user__github_username').annotate(total=Count('user')).order_by('-total')
